  ...
  void parseOptions(int argc, char* argv[]) {
    ...
    if (argc > optind) {
      for (int i=0; i<argc-optind; i++) {
		...
		std::string gstreamer_pipeline (int sensor_id, int capture_width, int captur    e_height, int display_width, int display_height, int framerate, int flip_met    hod) {
		...
		std::to_string(display_height) + ", format=(string)BGRx ! videoco    nvert ! video/x-raw, format=(string)BGR ! appsink";
		}

void gstreamer_setup() {
		int sensor_id = 1;
		int width = 320; // 320
		int height = 240; // 240 
		int exposure = 1;
