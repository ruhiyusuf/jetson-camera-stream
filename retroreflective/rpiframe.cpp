...
std::string bad_gstreamer_pipeline (int sensor_id, int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
		return "nvarguscamerasrc sensor-id=0 sensor-mode=4 ! 'video/x-raw(memory:NVMM), \
		format=NV12, width=(int)" + std::to_string(capture_width) + ", \
		height=(int)" \
		+ std::to_string(capture_height)
		+ ", framerate=(fraction)" + std::to_string(framerate); 
		//+ "' !  nvv4l2h264enc insert-sps-pps=true     ! \
		h264parse ! rtph264pay pt=96 ! appsink ";
}	
